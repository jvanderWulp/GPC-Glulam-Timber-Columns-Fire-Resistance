...
def thermal_restart(
    ...
    *,
    ...
    for i in range(1, max_restarts + 1):
        # Create a new restart input name
        ...
        # Determine expected .OUT filename for this restart
        ...
        # Check derivatives in the new .OUT file
...
def Temp_derivatives(file_path, node_id):
    ...
    results = []  # list of (time_seconds, temperature)
...
def make_restart_cross_section(workdir, filename, firstfile, firsttime, t_extend):
    ...
    if i_temperat == -1 and i_restartt == -1:
        raise ValueError("Neither 'TEMPERAT' or 'RESTARTT' found in the template .IN file.")
...
def extend_firecurve(workdir, time_s, T0):
    ...
    T0 : float | int
        Corresponding temperature (°C) at that time.
    """
    workdir = Path(workdir)
